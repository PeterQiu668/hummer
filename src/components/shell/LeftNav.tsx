...
export default function LeftNav() {
  ...
  const tenant = identityState?.identity.tenant;
  const tenants = identityState?.tenants ?? [];
    ...
    <>
      <aside className="absolute bottom-0 left-0 top-12 z-20 hidden w-60 flex-col border-r border-neutral-200 bg-neutral-25 md:flex">
        <div className="relative border-b border-neutral-200 p-3">
          <button type="button" onClick={() => setTenantOpen((value) => !value)} className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left hover:bg-neutral-100"><div className="grid h-7 w-7 place-items-center rounded-md bg-neutral-900 text-[11px] font-semibold text-white">{tenant?.name.slice(0, 1) ?? '企'}</div><div className="min-w-0 flex-1 leading-tight"><div className="truncate text-[12.5px] font-semibold text-neutral-900">{tenant?.name ?? '企业工作区'}</div><div className="mt-0.5 text-[10px] text-neutral-500">{identityState?.identity.account.displayName ?? '当前身份'}</div></div><ChevronDown size={13} className={`text-neutral-400 transition ${tenantOpen ? 'rotate-180' : ''}`} /></button>
          {tenantOpen && <div className="absolute left-3 right-3 top-[54px] z-40 rounded-md border border-neutral-200 bg-white p-1 shadow-lg">{tenants.map((item) => <button type="button" key={item.id} onClick={() => { void identityState?.switchTenant(item.id).then(() => window.location.reload()); setTenantOpen(false); }} className={`w-full rounded px-2 py-2 text-left text-[11.5px] ${tenant?.id === item.id ? 'bg-primary-50 text-primary-800' : 'text-neutral-600 hover:bg-neutral-50'}`}>{item.name}</button>)}</div>}
        </div>
